...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    if (event.type === 'checkout.session.completed' || event.type === 'invoice.paid') {
      ...
      const email = (session.customer_details?.email || session.customer_email || 'unknown@unknown').toLowerCase();

      const license = uuid().toUpperCase();
      const token = uuid().replace(/-/g, '').toUpperCase();

      await kv.set(`license:${email}`, JSON.stringify({ email, active: true, createdAt: Date.now(), sessionId: session.id }), { ex: 60 * 60 * 24 * 30 });
